                                ...
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        @foreach ($donHangs as $item)
                                            <tr>
                                                <th class="text-center"><a href="{{ route('donhangs.show', $item->id) }}">{{ $item->ma_don_hang }}</a></th>
                                                <td>{{ $item->created_at->format('d-m-Y') }}</td>
                                                <td>{{ $trangThaiDonHang[$item->trang_thai_don_hang] }}</td>
                                                <td>{{ number_format($item->tong_tien, 0, '', '.') }}</td>         
                                                <td>
                                                    <a class="btn btn-sqr" href="{{ route('donhangs.show', $item->id) }}">View</a>
                                                    ...
                                                    <form action="{{ route('donhangs.update', $item->id) }}" method="POST" class="d-inline">
                                                        ...
                                                        @elseif($item->trang_thai_don_hang === $type_dang_van_chuyen)    
                                                            ...
                                                            <button type="submit" class="btn btn-sqr bg-success" onclick="return confirm('Bạn có xác nhận đã nhận hàng')">Đã nhận hàng</button>
                                                        @endif
                                                    </form>
                                                </td>                             
                                            </tr>  
